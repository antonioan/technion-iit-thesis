...
Construct the deterministic transducer $\cT=\tup{I,O,S,s_0,\delta,\lab}$ as in~\cref{fig:example_parikh_not_symbolwise}, where $I=O=[m]=\{0,\cdots, m-1\}$ (so the input and output alphabets are $2^I=2^O$).
% \begin{gather*}
%     S = \{ s_0, \mathrm{sink}, 
%         s_1, \dots, s_{k-1}, 
%         t_1, \dots, t_{k-1} \} \\
...
\label{example:gap}
At first glance, the transducer $\cT$ in~\cref{fig:example_gap} might look intimidating. However, it is in fact simple, and its round-by-round behaviour can once more be summarized in a table (see~\cref{tab:example_gap}). The proof for $\cT$ being Parikh round symmetric is left for the reader. To see that $\cT\nprec^{p,s}_k \cT^\pi$, consider the word $x=\{0\} \emptyset \emptyset$. The output of $\cT$ on $x$ is $0\emptyset 2$. All permutations $x'$ of $x$ either start with $\{1\}$ or $\emptyset$, their respective outputs being either $\{1,2\}\emptyset\emptyset$ or $\emptyset^3$. In all cases, we have $T(x')\not\equiv_s \pi(T(x))$.
...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
	\caption{Something about~\cref{example:gap}. The transitions $i\in\sigma$ and $i\notin\sigma$ mean all letters from $\sI$ that, respectively, contain or do not contain $i$.}
	\label{fig:example_gap}
\end{figure}

\begin{table}[!htb]
    \centering
    \caption{Something about~\cref{example:gap}.}
    \vspace{2mm}
    \def\arraystretch{1.3}
    \begin{tabular}{c|c}
        Input & Output \\
