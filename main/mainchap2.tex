...
% We now give a \PSPACE-hardness lower bound, thus concluding the problem is \PSPACE-complete. Actually, following~\autoref{lem:reduce_equiv_to_simulation}, we give a stronger lower bound for round equivalence.\footnote{The reduction in \autoref{lem:reduce_equiv_to_simulation} is a Turing reduction, so a mere lower bound for round equivalence does not establish a bound for round simulation. Thankfully, our \PSPACE-hardness proof actually explicitly shows the hardness of both simulation and equivalence.}
\begin{theorem}
