...
The above algorithm has given a way to check $k$-round-symmetry of a transducer $T$ w.r.t.\! any family of permutations $\Pi$ in $\PSPACE$. However, whenever we are interested in checking for the symmetry of a transducer, $k$ is usually not given. We must then look for ways to bound $k$ so as, to check for round-symmetry with \emph{any} round length, we need only check a finite number (hopefully no more than exponential in size) of possibilities for $k$.
...
\subparagraph*{The Redundant Product Construction}
...
\begin{figure}[ht]
    	\centering
    	\begin{tikzpicture}[shorten >=1pt,node distance=1.2cm and 5cm,on grid,auto]
    		\clip (-5,0.16) rectangle (10, -1.7);
    		\node (perm_k_1) {$\perm_{k'}(\cB_1)$}; 
    		\node (perm_k_2) [right=of perm_k_1] {$\perm_{k'}(\cB_2)$};
    		\node (perm_kp_1) [below=of perm_k_1] {$\perm_{k}(\cB_1)$};
    		\node (perm_kp_2) [right=of perm_kp_1]{$\perm_{k}(\cB_2)$};
     		...
     		\path[->] 
    		(perm_k_1)  edge node[left] {$\varphi$} (perm_kp_1)
    		(perm_kp_2)  edge node[right] {$\varphi^{-1}$} (perm_k_2)
    		(perm_kp_1)  edge node[fill=white, anchor=center, pos=0.5] {$\subseteq$} ($(perm_kp_2)+(-1.3cm,0cm)$);
    	\end{tikzpicture}
    	\caption{A diagram for the proof structure of \autoref{lem:profile_equality_to_simulation}.}
    	\label{fig:type_profile}
    \end{figure}

Combining \autoref{lem:type_profile_bound} and \autoref{lem:profile_equality_to_simulation}, we can effectively compute $K_0$ such that if $L(\cA^k_1)\subseteq L(\cA^k_2)$ for some $k$, then this holds for some $k<K_0$. Finally, using \autoref{lem:round_equivalence_iff_perm_containment}, this concludes the proof of \autoref{thm:bound_on_k}. \qed
...
\begin{figure}[ht]
	\centering
	\begin{tikzpicture}[shorten >=1pt,node distance=0.8cm and 2cm,on grid,auto]
    ...
    \end{tikzpicture}%
    \hspace{2cm}%
    \begin{tikzpicture}[shorten >= 1mm, node distance=1.0cm and 1.5cm,on grid,auto]
        \tikzstyle{smallnode}=[circle, inner sep=0mm, outer sep=0mm, minimum size=3mm, draw=black];
    
        \node[smallnode] (q_0) {}; 
        \node[smallnode, label={$s^1_2$}] (q_b1) [right=of q_0] {}; 
        \node[smallnode, label={$s^2_2$}] (q_b2) [right=of q_b1] {}; 
        \node[smallnode, label={$s^3_2$}] (q_b3) [right=of q_b2] {}; 
        \node[smallnode, label={$s^1_1$}] (q_a1) [above=of q_b1] {}; 
        \node[smallnode, label={$s^2_1$}] (q_a2) [right=of q_a1] {}; 
        \node[smallnode, label={$s^1_3$}] (q_c1) [below=of q_b1] {}; 
        \node[smallnode, label={$s^2_3$}] (q_c2) [right=of q_c1] {}; 
        \node[smallnode, label={$s^3_3$}] (q_c3) [right=of q_c2] {}; 
        \node[smallnode, label={$s^4_3$}] (q_c4) [right=of q_c3] {}; 
        \node[smallnode, label={$s^5_3$}] (q_c5) [right=of q_c4] {}; 
        \path[->] 
        (q_0) edge node {$1$} (q_a1)
              edge node {$2$} (q_b1)
              edge node {$3$} (q_c1)
        (q_a1) edge node {$\cP$} (q_a2)
        (q_a2) edge [out=200, in=30, red,pos=0.2] node {$\varepsilon$} (q_0)
        (q_b1) edge node {$\cP$} (q_b2)
        (q_b2) edge node {$\cP$} (q_b3)
        (q_b3) edge [out=190, in=-10, red, pos=0.2] node {$\varepsilon$} (q_0)
        (q_c1) edge node {$\cP$} (q_c2)
        (q_c2) edge node {$\cP$} (q_c3)
        (q_c3) edge node {$\cP$} (q_c4)
        (q_c4) edge node {$\cP$} (q_c5)
        (q_c5) edge [out=150, in=-15, red, pos=0.1,above] node {$\varepsilon$} (q_0);
    \end{tikzpicture}
	\caption{The transducers $\cT_1$ (left) and $\cT_2$ (right) for $m=3$ in \autoref{example:exponential_round_length}. The $\varepsilon$-edge from a state $s^{p_i}_i$ to $s_0$ in $\cT_2$ mean that the transition function from state $s^{p_i}_i$ behaves identically as from $s_0$.}
	\label{fig:prime}
\end{figure}
